...
public class GHTree extends TreeIndex {
    ...
    protected TreeNode buildTreeRecursive(List<MetricSpaceData> data, int depth) {
        ...
        if (config.isVerbose()) {
            System.out.printf("  深度%d: 选择支撑点 p1=%s, p2=%s%n", 
                    depth, pivot1.toString(), pivot2.toString());
        }
        ...
        if (config.isVerbose()) {
            System.out.printf("  深度%d: 数据划分完成，左=%d，右=%d%n", 
                    depth, leftData.size(), rightData.size());
        }
    ...
    private void rangeQueryRecursive(TreeNode node, MetricSpaceData queryObject, 
        ...
        if (node.isLeaf()) {
            ...
            if (!(d1 - d2 > 2 * radius)) {
                ...
                System.out.printf("    剪枝左子树 (d1-d2=%.3f > 2r=%.3f)%n", 
                        d1 - d2, 2 * radius);
            }
            ...
            if (!(d2 - d1 > 2 * radius)) {
                ...
                System.out.printf("    剪枝右子树 (d2-d1=%.3f > 2r=%.3f)%n", 
                        d2 - d1, 2 * radius);
            }
        }
    }
        ...
        if (node.isLeaf()) {
            ...
            if (leftFirst) {
                ...
                if (!(d1 - d2 > 2 * currentRadius[0])) {
                    knnQueryRecursive(internal.getLeftChild(), queryObject, k, knnHeap, currentRadius);
                }
                // 再访问右子树
                if (!(d2 - d1 > 2 * currentRadius[0])) {
                    knnQueryRecursive(internal.getRightChild(), queryObject, k, knnHeap, currentRadius);
                }
            } else {
                // 先访问右子树
                if (!(d2 - d1 > 2 * currentRadius[0])) {
    ...
    protected String getNodeDescription(TreeNode node) {
        if (node.isLeaf()) {
            ...
            GHInternalNode internal = (GHInternalNode) node;
            return String.format("GH内部节点 [左=%d, 右=%d, p1=ID%d, p2=ID%d]",
                    internal.getLeftSize(), internal.getRightSize(),
