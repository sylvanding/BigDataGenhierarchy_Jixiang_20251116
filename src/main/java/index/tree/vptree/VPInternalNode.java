...
public class VPInternalNode extends InternalNode {
    ...
    public VPInternalNode(MetricSpaceData pivot,
                          TreeNode innerChild, TreeNode outerChild,
                          DistanceRange innerRange, DistanceRange outerRange,
                          double medianDistance, int depth) {
    ...
    public String getNodeInfo() {
        return String.format("VPInternalNode[depth=%d, inner=%d%s, outer=%d%s, median=%.4f, pivot=%s]",
                depth, getInnerSize(), getInnerRange(),
                getOuterSize(), getOuterRange(),
                medianDistance, pivots.get(0).toString());
    }
}
