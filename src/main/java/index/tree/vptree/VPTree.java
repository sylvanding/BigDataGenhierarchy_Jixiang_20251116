...
public class VPTree extends TreeIndex {
    ...
    protected TreeNode buildTreeRecursive(List<MetricSpaceData> data, int depth) {
        ...
        for (MetricSpaceData obj : data) {
            if (obj.getDataId() == pivot.getDataId()) continue; // 跳过pivot本身
        ...
        if (config.isVerbose()) {
            System.out.printf("  深度%d: 数据划分完成，内球=%d%s，外球=%d%s，中位数=%.4f%n",
                    depth, innerData.size(), innerRange, outerData.size(), outerRange, medianDistance);
        }
        ...
        return new VPInternalNode(pivot, innerChild, outerChild, 
                innerRange, outerRange, medianDistance, depth);
    }
     ...
     */
    private void rangeQueryRecursive(TreeNode node, MetricSpaceData queryObject,
                                     double radius, List<MetricSpaceData> results) {
        nodeAccesses++;
        ...
        if (node.isLeaf()) {
            ...
            boolean visitInner = !(dq + radius < innerRange.lower || dq - radius > innerRange.upper);
            
            if (visitInner) {
                rangeQueryRecursive(internal.getInnerChild(), queryObject, radius, results);
            } else if (config.isVerbose()) {
                System.out.printf("    剪枝内球子树 (dq=%.3f, r=%.3f, range=%s)%n",
                        dq, radius, innerRange);
            }
            
            // 检查是否需要访问外球子树
            DistanceRange outerRange = internal.getOuterRange();
            boolean visitOuter = !(dq + radius < outerRange.lower || dq - radius > outerRange.upper);
            
            if (visitOuter) {
                rangeQueryRecursive(internal.getOuterChild(), queryObject, radius, results);
            } else if (config.isVerbose()) {
                System.out.printf("    剪枝外球子树 (dq=%.3f, r=%.3f, range=%s)%n",
                        dq, radius, outerRange);
            }
        }
    }
    ...
    public List<MetricSpaceData> knnQuery(MetricSpaceData queryObject, int k) {
        PriorityQueue<KNNResult> knnHeap = new PriorityQueue<>(
    ...
    private void knnQueryRecursive(TreeNode node, MetricSpaceData queryObject, int k,
        ...
        if (node.isLeaf()) {
            ...
            if (innerFirst) {
                // 先访问内球
                DistanceRange innerRange = internal.getInnerRange();
                if (!(dq + currentRadius[0] < innerRange.lower || dq - currentRadius[0] > innerRange.upper)) {
                    knnQueryRecursive(internal.getInnerChild(), queryObject, k, knnHeap, currentRadius);
                }
                // 再访问外球
                DistanceRange outerRange = internal.getOuterRange();
                if (!(dq + currentRadius[0] < outerRange.lower || dq - currentRadius[0] > outerRange.upper)) {
                    knnQueryRecursive(internal.getOuterChild(), queryObject, k, knnHeap, currentRadius);
                }
            } else {
                // 先访问外球
                DistanceRange outerRange = internal.getOuterRange();
    ...
    protected String getNodeDescription(TreeNode node) {
        if (node.isLeaf()) {
            ...
            return String.format("VP内部节点 [内=%d, 外=%d, median=%.3f, pivot=ID%d]",
                    internal.getInnerSize(), internal.getOuterSize(),
                    internal.getMedianDistance(), internal.getPivot().getDataId());
        }
    }
}
